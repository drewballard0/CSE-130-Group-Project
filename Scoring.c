...
int main() {
	int combo[] = {1,1,4,1,1}; //take in roll values
	...
	while (round_number < 13) { //ensure only 13 rounds are played
		...
		if (check[choice] == true) { //check to see if category has already been used
			...
			switch(choice) { //based on user input, assigns points
				case 1:
					printf("Score is: %d", aces(user, combo)); //finds score
					check[1] = true; //prevent category from being chosen again
					round_number++; //iterate to make sure only 13 rounds are played
					system("clear"); //clear screen
				break;
				case 2:
					printf("Score is: %d", twos(user, combo));
					check[2] = true;
					round_number++;
					system("clear");
				break;
				case 3:
					printf("Score is: %d", threes(user, combo));
					check[3] = true;
					round_number++;
					system("clear");
				break;
				case 4:
					printf("Score is: %d", fours(user, combo));
					check[4] = true;
					round_number++;
					system("clear");
				break;
				case 5:
					printf("Score is: %d", fives(user, combo));
					check[5] = true;
					round_number++;
					system("clear");
				break;
				case 6:
					printf("Score is: %d", sixes(user, combo));
					check[6] = true;
					round_number++;
					system("clear");
				break;
